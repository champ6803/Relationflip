    ...
    $.ajax({
        ...
        success: function (data) {
            if (data === 'success') {
                ...
                sendEmail();
                sendEmailConfirm(date, employee_id, start, end);
                saveEmailQueue();
...
sendEmailConfirm = function (appointment_date, employee_id, start_time, end_time) {
    var token = $('#token').val();
    var ad = appointment_date.split('-');
    var a_date = ad[2] + "-" + ad[1] + "-" + ad[0];
    $.ajax({
        type: 'post',
        url: 'mailMeetingConfirm',
        async: true,
        data: {
            '_token': token,
            'employee_id': employee_id,
